...
SAM3 worker 复用固定文本特征，并以 mmap/assign 降低权重加载的 CPU 内存。一次真实 352 帧序列的全部 72 个语义帧对照中，SAM3 阶段耗时减少 6.2%、该进程峰值 RSS 降低 41.0%；掩码、裁图和补全前融合地图一致。这是单场景阶段测量，详见 [性能报告](docs/performance-20260924.md)。
